package com.lining.yokinjar.ErJi;
...
public class SortDetailFragment extends BaseFragment<SortDetailPresenter, String> implements CheckListener {
    ...
    private void smoothMoveToPosition(int n) {
        ...
        Log.d(TAG, String.valueOf(firstItem));
        Log.d(TAG, String.valueOf(lastItem));
        if (n <= firstItem) {
            mRv.scrollToPosition(n);
        } else if (n <= lastItem) {
            Log.d(TAG, String.valueOf(n) + "VS" + firstItem);
            int top = mRv.getChildAt(n - firstItem).getTop();
            Log.d(TAG, String.valueOf(top));
            mRv.scrollBy(0, top);
        } else {
            mRv.scrollToPosition(n);
            move = true;
